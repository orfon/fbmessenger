...
//  Copyright 2017 Philipp Naderer-Puiu / ORF Online und Teletext GmbH & Co KG
//
 ...
 * @param {string} notificationType push notification type, defaults to `REGULAR`.
 * @param {boolean} sharable if false it disables the share button in Messenger client, defaults to true
 * @param {string} imageAspectRatio aspect ratio used to render images, must be `horizontal` or `square`, defaults to `horizontal`
 * @param {string} tag message tag to send outside the 24+1 time window
 ...
 * ]);
 */
FBMessenger.prototype.sendGenericTemplate = function(id, elements, sharable, imageAspectRatio, notificationType, tag) {
    return this._request("me/messages", {
        "messaging_type": MESSAGING_TYPE.RESPONSE,
        "recipient": {
            "id": id
        ...
        "message": {
            "attachment": {
                ...
                "payload": {
                    ...
                    "image_aspect_ratio": typeof imageAspectRatio === "string" ? imageAspectRatio : "horizontal",
                    "elements": elements
                }
            }
        },
 ...
 * ]);
 */
FBMessenger.prototype.sendButtonTemplate = function(id, text, buttons, sharable, notificationType, tag) {
    return this._request("me/messages", {
        "messaging_type": MESSAGING_TYPE.RESPONSE,
        "recipient": {
            "id": id
...
/**
 * Sets the greeting text for the start of a conversation with the bot.
 *
 ...
 * bot.setGetStartedButton(payloadString);
 */
 ...
 *
 * _This method changed from 2.x to 3.x due the new Messenger APIs._
 *
...
/**
 * Allows to specify an `account_linking_url</url>
 ...
 * @see <a href="https://developers.facebook.com/docs/messenger-platform/insights/page-messaging">Messenger Platform - Page Messaging Insights</a>
 * @example const duatc = bot.getDailyUniqueActiveThreads();
 */
FBMessenger.prototype.getDailyUniqueActiveThreads = function() {
    return this._request("me/insights/page_messages_active_threads_unique", null, null, "GET");
};

/**
 * Retrieves the daily unique conversation count via the Pages Insights API.
 * @see <a href="https://developers.facebook.com/docs/messenger-platform/insights/page-messaging">Messenger Platform - Page Messaging Insights</a>
 * @example const duc = bot.getDailyUniqueConversations();
 */
FBMessenger.prototype.getDailyUniqueConversations = function() {
    return this._request("me/insights/page_messages_feedback_by_action_unique", null, null, "GET");
};

/**
 * Get the messenger code for the current page.
 * @param {string} type optional the code type to generate; default is `standard`
 * @param {string} imageSize optional image size of the code; defaults to 1000
 * @param {string} ref optional referral parameter containing only `a-z A-Z 0-9 +/=-.:_`
 * @returns {string} the URL of the Messenger code or null
 * @see <a href="https://developers.facebook.com/docs/messenger-platform/messenger-code">Messenger Platform - Messenger Code</a>
 * @example const url = bot.getMessengerCode();
 */
FBMessenger.prototype.getMessengerCode = function(type, imageSize, ref) {
    const codeType = typeof type === "string" ? type : "standard";
    const codeSize = Number.isSafeInteger(imageSize) && imageSize <= 2000 && imageSize > 100 ? imageSize : 1000;
    ...
    const response = this._request("me/messenger_codes", payload, null, "POST");
    return typeof response.uri === "string" ? response.uri : null;
};

/**
 * Uploads an reusable attachment that you may later send out to many users. Even if the method is named
 * _uploadAttachment_ you must specify it as an URL and not as an multipart attachment.
 * @param {string} type media type of the attachment, must be `image`, `audio`,
 *                      `video`, or `file`
 * @param {string} url url of the attachment where the Messenger platform can fetch from
 * @returns {string} the reusable non-expiring attachment id
 * @see <a href="https://developers.facebook.com/docs/messenger-platform/send-api-reference/attachment-upload">Messenger Platform - Attachment Upload</a>
 * @example const attachmentId = bot.uploadAttachment("image", "https://example.org/foo.jpg");
 */
FBMessenger.prototype.uploadAttachment = function(type, url) {
    const response = this._request("me/message_attachments", {
        "message": {
            "attachment": {
                "type": type,
                "payload":{
